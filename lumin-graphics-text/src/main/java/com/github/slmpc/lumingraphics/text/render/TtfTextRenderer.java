...
public final class TtfTextRenderer implements TextRenderer {
    ...
    public TtfTextRenderer(TextBatchSink sink) {
        this(new TextLayoutEngine(), 1.0f, sink);
    }
    ...
    @Override
    public synchronized TextMeasurement measure(String text, float scale, TtfFontLoader font) {
        ensureOpen();
        return layouts.measure(text, effectiveScale(scale), font);
    }

    @Override
    public synchronized TextLayout add(String text, float x, float y, float scale, TtfFontLoader font) {
        return add(text, x, y, scale, WHITE, font);
    }

    @Override
    public synchronized TextLayout add(
    ...
    @Override
    public synchronized TextLayout addRotated(
            String text, float x, float y, float scale, LuminColor color, TtfFontLoader font,
    ...
    private void ensureOpen() {
        if (closed) throw new FontClosedException("Text renderer is closed");
    }
