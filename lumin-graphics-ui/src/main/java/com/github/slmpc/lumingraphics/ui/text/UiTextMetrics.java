...
public interface UiTextMetrics {
    ...
    default float textWidth(String text, float scale, String fontId) { return measure(text, scale, fontId).width(); }
    default float textHeight(float scale, String fontId) { return measure("Mg", scale, fontId).height(); }
    static UiTextMetrics of(UiTextMetrics metrics) {
        Objects.requireNonNull(metrics, "metrics");
        return (text, scale, fontId) -> {
            if (text == null || !Float.isFinite(scale) || scale <= 0) throw new IllegalArgumentException("text metric input is invalid");
    ...
    static UiTextMetrics renderer(TextRenderer renderer, UiResourceResolver resources) {
        Objects.requireNonNull(renderer, "renderer"); Objects.requireNonNull(resources, "resources");
    ...
    record Measurement(float width, float height) {
        public Measurement { if (!Float.isFinite(width) || !Float.isFinite(height) || width < 0 || height < 0) throw new IllegalArgumentException("measurement is invalid"); }
    }
}
