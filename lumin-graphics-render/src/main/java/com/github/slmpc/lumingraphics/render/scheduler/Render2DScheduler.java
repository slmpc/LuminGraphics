...
public final class Render2DScheduler implements AutoCloseable {
    ...
    public LayerHandle layer(int layer) {
        requireOpen();
        return layers.computeIfAbsent(layer, key -> new LayerHandle(key));
    }
    ...
    public void clear() {
        requireOpen();
        commands.clear();
        flushedSequences.clear();
        layers.values().forEach(LayerHandle::clearScissors);
        sequence = 0;
    }
    ...
    public final class LayerHandle {
        ...
        private LayerHandle(int layer) {
            this.layer = layer;
        }

        public void pushScissor(Render2DScissor scissor) {
            requireOpen();
            if (scissor == null) throw new IllegalArgumentException("scissor is null");
            scissors.push(scissors.isEmpty() ? scissor : scissors.peek().intersect(scissor));
        }

        public void popScissor() {
            requireOpen();
            if (scissors.isEmpty()) throw new IllegalStateException("scissor stack is empty");
            scissors.pop();
        }
        ...
        public void addRect(Render2DBounds bounds, LuminColor color) {
            add(new Render2DCommand.Rect(layer, nextSequence(), bounds, scissor(), color));
        }
        ...
        public void addRoundRect(Render2DBounds b, float radius, LuminColor color) {
            add(new Render2DCommand.RoundRect(layer, nextSequence(), b, scissor(), radius, color));
        }
        ...
        public void addOutline(Render2DBounds b, float radius, float width, LuminColor color) {
            add(new Render2DCommand.RoundRectOutline(layer, nextSequence(), b, scissor(), radius, width, color));
        }
        ...
        public void addShadow(Render2DBounds b, float radius, float blur, LuminColor color) {
            add(new Render2DCommand.Shadow(layer, nextSequence(), b, scissor(), radius, blur, color));
        }
        ...
        public void addTexture(Render2DBounds b, Render2DTexture texture, LuminColor color) {
            add(new Render2DCommand.Texture(layer, nextSequence(), b, scissor(), texture, color));
        }
        ...
        public void addGlyphs(Render2DBounds b, Render2DTexture texture, List<GlyphQuad> glyphs) {
