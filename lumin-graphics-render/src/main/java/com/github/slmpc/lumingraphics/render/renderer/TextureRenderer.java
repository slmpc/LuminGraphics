...
public final class TextureRenderer extends AbstractRenderer<Render2DCommand.Texture> {
    public TextureRenderer(RenderResources r, int c) {
        super(Render2DCommand.Texture.class, r, c, "texture");
    }

    @Override
    protected VertexBatch vertices(Render2DCommand.Texture c) {
        return VertexBatches.texture(c);
    }

    @Override
    protected Render2DTexture texture(Render2DCommand.Texture c) {
        return c.texture();
    }
}
