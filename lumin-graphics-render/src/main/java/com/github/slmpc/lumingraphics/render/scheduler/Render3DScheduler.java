...
public final class Render3DScheduler {
    ...
    public void add(int priority, Render3DAction action) {
        ...
        commands.add(new Command(priority, sequence++, action)); flushed = false;
    }
    public boolean isEmpty() { return commands.isEmpty(); }
    public void clear() { commands.clear(); sequence = 0; flushed = false; }
    ...
    public void flushAndClear(RenderExecution execution) { try { flush(execution); } finally { clear(); } }
    @FunctionalInterface public interface Render3DAction { void render(RenderExecution execution); }
