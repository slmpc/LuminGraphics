...
public final class GlyphBatchRenderer extends AbstractRenderer<Render2DCommand.Glyphs> {
    public GlyphBatchRenderer(RenderResources r, int c) {
        super(Render2DCommand.Glyphs.class, r, c, "ttf-font-aa");
    }

    @Override
    protected VertexBatch vertices(Render2DCommand.Glyphs c) {
        return VertexBatches.glyphs(c.glyphs(), c.originX(), c.originY(), c.rotationDegrees());
    }

    @Override
    protected Render2DTexture texture(Render2DCommand.Glyphs c) {
        return c.texture();
    }
}
