...
class Todo11MigrationSurfaceTest {
    ...
    @Test
    @DisplayName("MIG-RENDER-PIPELINES")
    ...
    @Test
    @DisplayName("MIG-INJECTED-RENDER-CONTEXT")
    ...
    @Test
    @DisplayName("MIG-INJECTED-TEXTURE-RESOLVER")
    ...
    @Test
    @DisplayName("MIG-CORE-VERTEX-FORMATS")
    ...
    @Test
    @DisplayName("MIG-CORE-BUFFER-WRITES")
    ...
    @Test
    @DisplayName("MIG-CORE-RING-BUFFER")
    ...
    @Test
    @DisplayName("MIG-RENDER-IMMEDIATE")
    ...
    @Test
    @DisplayName("MIG-RENDER-TESSELLATOR")
    void tessellatorExpandsQuadToSixExactVertices() {
        ...
        assertEquals(96, batch.bytes().remaining());
    }

    @Test
    @DisplayName("MIG-RENDER-LIFECYCLE")
    void rendererLifecycleRejectsCloseDuringFrame() {
        ...
        set.endFrame();
        set.close();
        assertTrue(set.allFramesEnded());
    }

    @Test
    @DisplayName("MIG-RECT-BATCHING")
    void adjacentRectsShareOneUploadAndDraw() {
        Fixture f = fixture();
    ...
    @Test
    @DisplayName("MIG-ROUND-OUTLINE")
    void outlineUsesExactPipeline() {
        assertCommand("round-rectangle-outline", f -> f.scheduler.layer(0).addOutline(bounds(), 2, 1, WHITE));
    }

    @Test
    @DisplayName("MIG-ROUND-RECT")
    void roundRectUsesExactPipeline() {
        assertCommand("round-rectangle", f -> f.scheduler.layer(0).addRoundRect(bounds(), 2, WHITE));
    }

    @Test
    @DisplayName("MIG-SHADOW-BATCHING")
    void shadowUsesExpandedBoundsAndPipeline() {
        assertCommand("shadow", f -> f.scheduler.layer(0).addShadow(bounds(), 2, 3, WHITE));
    }

    @Test
    @DisplayName("MIG-GLYPH-BATCHING")
    ...
    @Test
    @DisplayName("MIG-TEXTURE-BATCHING")
    void textureBindsItsInjectedDescriptor() {
        Fixture f = fixture();
        f.scheduler.layer(0).addTexture(bounds(), new Render2DTexture.Resource("atlas"), WHITE);
        f.scheduler.flush(f.execution());
        assertTrue(f.fake.trace().contains("descriptor=atlas"));
        f.scheduler.close();
    }

    @Test
    @DisplayName("MIG-TRIANGLE-BATCHING")
    void triangleUsesThreeVertices() {
        Fixture f = fixture();
        f.scheduler.layer(0).addTriangle(5, 5, 2, WHITE);
        f.scheduler.flush(f.execution());
        assertTrue(f.fake.trace().contains("draw=3"));
        f.scheduler.close();
    }

    @Test
    @DisplayName("MIG-SCHEDULER-BOUNDS")
    void boundsUseStrictIntersection() {
        assertTrue(new Render2DBounds(0, 0, 2, 2).intersects(new Render2DBounds(1, 1, 2, 2)));
        assertFalse(new Render2DBounds(0, 0, 1, 1).intersects(new Render2DBounds(1, 0, 1, 1)));
    }

    @Test
    @DisplayName("MIG-SCHEDULER-COMMAND")
    void commandRecordsCaptureSubmissionScissor() {
        Fixture f = fixture();
        var layer = f.scheduler.layer(0);
        layer.pushScissor(new Render2DScissor(1, 2, 3, 4));
        layer.addRect(bounds(), WHITE);
        layer.popScissor();
        f.scheduler.flush(f.execution());
        assertTrue(f.fake.trace().contains("scissor=1,2,3,4"));
        f.scheduler.close();
    }

    @Test
    @DisplayName("MIG-SCHEDULER-KINDS")
    void commandKindsCoverAllRendererRoutes() {
    ...
    @Test
    @DisplayName("MIG-SCHEDULER-ORDER")
    void schedulerOrderIsCoveredByGoldenTrace() {
        new Todo11BehaviorTest().schedulerSortsLayersStablyCullsAndIntersectsNestedScissors();
    }

    @Test
    @DisplayName("MIG-SCHEDULER-SCISSOR")
    void invertedAndDisjointScissorsFail() {
    ...
    @Test
    @DisplayName("MIG-SCHEDULER-TEXTURE")
    void textureReferencesValidateIds() {
    ...
    @Test
    @DisplayName("MIG-SCHEDULER-3D")
    void scheduler3dPriorityHasStableGoldenOrder() {
        new Todo11BehaviorTest().threeDimensionalCommandsUsePriorityThenStableInsertionOrder();
    }

    @Test
    @DisplayName("MIG-SHADER-BLUR")
    void blurBindsInputAndPipeline() {
        assertEffect("blur", (f, e) -> {
            try (var s = new BlurShader(f.resources(), 256)) {
                s.apply(e, new Render2DTexture.Resource("input"));
            }
        });
    }

    @Test
    @DisplayName("MIG-SHADER-FILTER")
    void filterBindsInputAndPipeline() {
        assertEffect("filter", (f, e) -> {
            try (var s = new FilterShader(f.resources(), 256)) {
                s.apply(e, new Render2DTexture.Resource("input"));
            }
        });
    }
    ...
    @Test
    @DisplayName("MIG-SHADER-FXAA")
    void fxaaBindsInputAndPipeline() {
        assertEffect("fxaa", (f, e) -> {
            try (var s = new FxaaShader(f.resources(), 256)) {
                s.apply(e, new Render2DTexture.Resource("input"));
            }
        });
    }
    ...
    @Test
    @DisplayName("MIG-SHADER-SANDBOX")
    void sandboxRejectsNonSandboxPipelineAndDrawsMenu() {
        FakeRhi fake = new FakeRhi();
        assertThrows(IllegalArgumentException.class, () -> new GlslSandbox(fake.resources(), 256, "rectangle"));
    ...
    @Test
    void missingShaderFailsBeforeBindingOrDraw() {
        Fixture f = fixture();
        f.fake.missingPipeline("rectangle");
        f.scheduler.layer(0).addRect(bounds(), WHITE);
        assertThrows(IllegalStateException.class, () -> f.scheduler.flush(f.execution()));
        assertTrue(f.fake.trace().stream().noneMatch(value -> value.startsWith("draw=")));
        f.scheduler.close();
    ...
    private static void assertCommand(String pipeline, java.util.function.Consumer<Fixture> submit) {
        ...
        submit.accept(f);
        f.scheduler.flush(f.execution());
        assertEquals(List.of(pipeline), f.fake.boundPipelines());
        f.scheduler.close();
    }

    private static void assertEffect(String pipeline, EffectCall call) {
        FakeRhi fake = new FakeRhi();
        call.run(fake, fake.execution(1, 0, 8, 8));
        assertEquals(List.of(pipeline), fake.boundPipelines());
        assertTrue(fake.trace().contains("descriptor=input"));
    }

    private static Fixture fixture() {
        FakeRhi fake = new FakeRhi();
        return new Fixture(fake, new Render2DScheduler(RendererSet.create(fake.resources(), 4096), 2));
    }

    private static Render2DBounds bounds() {
        return new Render2DBounds(1, 1, 4, 4);
    }

    private record Fixture(FakeRhi fake, Render2DScheduler scheduler) {
        RenderExecution execution() {
            return fake.execution(1, 0, 20, 20);
        }
    }

    @FunctionalInterface
    private interface EffectCall {
        void run(FakeRhi fake, RenderExecution execution);
    }
}
